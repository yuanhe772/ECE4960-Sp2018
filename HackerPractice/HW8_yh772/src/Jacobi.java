...
public class Jacobi {
	...
	public static int solver(SparseMatrix A, Vector b) {
		...
		while (convNorm > Math.pow(10, -7)) {
			...
			System.out.println("The "+i+"th iteration's normalized-second-norm error: ||A*x - b|| / ||b|| = "+currNN);
			
			// Iff the precision decreases with more iteration
			if(currNN < prevNN) {flag = false;}
		}		
		System.out.println("\n  To converge to |delta K| / |K| < 10^-7, it took iteration times of "+i);
		System.out.println("\n  [k0, k1, k2, k3, k4, k5] = "+Arrays.toString(currX.v));
